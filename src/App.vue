<template>
  <div style="height: 100vh">
    <yandex-map ref="map" style="width: 100%; height: 100%" :zoom="15" :coords="[59.132296, 37.845130]" :controls="[]">
    </yandex-map>
  </div>
</template>
...
export default {
  ...
  created() {
    const { Latitude: latitudes, Longitude: longitudes,
      Timestamp: locTimestamp, LocoType: locoType,
      LocoNumber: locoNumber } = location;
    ...
    this.points = latitudes.map((latitude, index) => ({
      ...
      locationTime: this.getLocationTime(this.getDiffTimestamp(locTimestamp[index],
        tempTimestamps), outsideTemps)
    }));
  },
  ...
  async mounted() {
    ...
    this.$refs.map.myMap?.controls.add(descriptionLoco, { floatIndex: 0 });
